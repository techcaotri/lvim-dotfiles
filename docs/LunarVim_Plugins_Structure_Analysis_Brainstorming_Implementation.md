...
flowchart TB
    ...
    subgraph NewStack["NEW stack -- lvim-new (NVIM_APPNAME=lvim-lazyvim)"]
        LauncherNew["Launcher (~/.local/bin/lvim-new)<br/>export NVIM_APPNAME + VIMRUNTIME<br/>then exec -a lvim-new (built nvim)"]
...
| Launcher | `~/.local/bin/lvim` (`exec -a "$NVIM_APPNAME" nvim -u .../init.lua`) | `~/.local/bin/lvim-new`, generated by `setup_lvim.sh:118-136` (`export ... ; exec -a lvim-new ...`) |
| `NVIM_APPNAME` | `lvim` | `lvim-lazyvim` (`setup_lvim.sh:20`) |
| Process name seen by tmux | `lvim` (argv[0] rewritten by `exec -a`) | `lvim-new` (argv[0] rewritten by `exec -a`) |
...
| tmux router | `~/.dotfiles/bin/tol` — sockets `lvim.PID.0`, panes whose command contains `lvim`, log `/tmp/tol.log` | `~/.dotfiles/bin/tol-new` — sockets `lvim-lazyvim.PID.0`, panes whose command contains `lvim-new`, log `/tmp/tol-new.log` |

The one row that reads like an accident but is not: **process name**. Both launchers use
`exec -a <name>` to rewrite `argv[0]`, so `tmux`'s `#{pane_current_command}` reports `lvim`
for LunarVim and `lvim-new` for the new editor. The subtlety worth internalizing is *which*
name that changes: `exec -a` rewrites **`argv[0]`** (`/proc/PID/cmdline`, which is what tmux
reads), but **not** `/proc/PID/comm` — so `ps -o comm` still prints `nvim` for a running
`lvim-new`, while `ps -o args` and tmux both print `lvim-new`. Verified on this machine:
`comm` = `nvim`, `argv[0]` = `lvim-new`.

Note the consequence for the two routers, because it is the opposite of what it looks like:
the globs are **not** mutually exclusive. `tol:74` matches `*"lvim"*`, and the string
`lvim-new` *contains* `lvim`, so `tol` will happily accept an `lvim-new` pane as a
candidate. What actually stops it stealing that pane is the layer below — `tol` only globs
LunarVim sockets (`lvim.*.0`, which cannot match `lvim-lazyvim.*.0`) and then requires the
pane's process tree to own one of them. The pane-command test is only a cheap prefilter; the
socket/PID correlation is the real discriminator (III.15).
...
- `~/.local/bin/lvim-new` (generated by `setup_lvim.sh:114`–`:131`):
  `export NVIM_APPNAME="lvim-lazyvim"; export VIMRUNTIME=".../neovim/runtime"; exec -a lvim-new ".../neovim/build/bin/nvim" "$@"`
...
- **`exec -a lvim-new ... "$@"`** means no wrapper process survives, argv[0] is rewritten to
  `lvim-new`, and every argument is forwarded verbatim.
...
flowchart TD
    ...
    RtBranch -- "yes (build tree)" --> LauncherA["BRANCH A launcher (:118-127)<br/>export NVIM_APPNAME=&quot;lvim-lazyvim&quot;<br/>export VIMRUNTIME=&quot;.../neovim/runtime&quot;<br/>exec -a lvim-new &quot;.../build/bin/nvim&quot; &quot;$@&quot;"]
    RtBranch -- "no (installed nvim)" --> LauncherB["BRANCH B launcher (:129-136)<br/>export NVIM_APPNAME=&quot;lvim-lazyvim&quot;<br/>exec -a lvim-new &quot;/usr/bin/nvim&quot; &quot;$@&quot;<br/>(no VIMRUNTIME)"]
...
# fully isolated from LunarVim via NVIM_APPNAME=lvim-lazyvim.
export NVIM_APPNAME="lvim-lazyvim"
exec -a lvim-new "/usr/bin/nvim" "$@"
```

Three properties of this shape are worth naming explicitly, because downstream components depend on
all of them:

1. **`exec -a lvim-new ...`** -- no wrapper process survives, and `argv[0]` is rewritten to
   `lvim-new`. tmux reads `/proc/PID/cmdline`, so `#{pane_current_command}` reports `lvim-new`,
   which is what `tol-new` matches on. Note `exec -a` does *not* change `/proc/PID/comm`, so
   `ps -o comm` still shows `nvim` (see III.15).
...
3. **A separate launcher name.** `lvim` and `lvim-new` are two distinct files in `~/.local/bin`, each
   `exec -a`-ing its own binary with its own `NVIM_APPNAME`. `setup_lvim.sh old` deletes only the
   latter.
...
sequenceDiagram
    ...
    Launcher->>Nvim: export NVIM_APPNAME=lvim-lazyvim<br/>export VIMRUNTIME=~/Dev/Playground_Terminal/neovim/runtime<br/>exec -a lvim-new .../neovim/build/bin/nvim "$@"
...
flowchart TD
    ...
    CmdFilter{"pane_current_command<br/>contains 'lvim-new' (or legacy 'nvim')?<br/>(tol-new:78)"}
...
sequenceDiagram
    ...
    TolNew->>TolNew: "filter: cmd contains 'lvim-new' (tol-new:78)"
...
| 2 | Pane command filter | `[[ $cmd != *"lvim"* ]]` (tol:74) | `[[ $cmd != *"lvim-new"* && $cmd != *"nvim"* ]]` (tol-new:78) | Both launchers end with `exec -a <name> ... nvim`, which rewrites `argv[0]` -- and therefore tmux's `#{pane_current_command}` -- to `lvim` and `lvim-new` respectively. `tol-new` also tolerates a bare `nvim` so that launchers generated before the `exec -a` change still resolve. The filters overlap (`lvim-new` contains `lvim`), so it is the socket/PID correlation in step 3, not this prefilter, that keeps the two routers apart. |
...
sequenceDiagram
    ...
    Tmux-->>TolNew: pane list -- keep only panes whose command contains "lvim-new"
    ...
    alt A running lvim-new server matches the socket
        ...
        Launcher->>Nvim: exec -a lvim-new (NVIM_APPNAME + VIMRUNTIME exported) -- client mode
        ...
        Launcher->>Nvim: exec -a lvim-new <build>/bin/nvim (NVIM_APPNAME + VIMRUNTIME exported)
...
6. **Pane matching.** `tol-new:78` keeps panes whose `pane_current_command` contains `lvim-new` (it also
   tolerates a bare `nvim`, so launchers generated before the `exec -a` change still resolve). Both launchers
   now use `exec -a <name>`, so argv[0] is literally `lvim` for LunarVim and `lvim-new` here. Note this
   rewrites `argv[0]` (`/proc/PID/cmdline`, what tmux reads) and *not* `/proc/PID/comm`, which still says
   `nvim`. The two filters do **overlap** -- `lvim-new` contains `lvim` -- so what actually keeps the two
   openers off each other's panes is the socket namespace plus the PID correlation in step 7, not this
   prefilter.
...
flowchart TD
    ...
    subgraph RuntimeLayer["Runtime layer (the editor itself)"]
        LauncherBin["Launcher (generated)<br/>~/.local/bin/lvim-new<br/>export NVIM_APPNAME + VIMRUNTIME<br/>exec -a lvim-new"]
...
|-----------|------|----------------|-----------------------|----------|
| lvim-new launcher | shell script (generated) | Export `NVIM_APPNAME` + `VIMRUNTIME`, exec built nvim as `lvim-new` | `exec -a lvim-new ... "$@"` | `~/.local/bin/lvim-new` |
