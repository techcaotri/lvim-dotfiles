...
setup_new() {
  ...
  # VIMRUNTIME when NVIM_BIN is a non-installed build tree.
...
exec env NVIM_APPNAME="$APPNAME" VIMRUNTIME="$NVIM_RUNTIME" "$NVIM_BIN" "\$@"
EOF
  else
    cat >"$LAUNCHER" <<EOF
#!/usr/bin/env bash
# Auto-generated by setup_lvim.sh -- launches the LazyVim migration config,
# fully isolated from LunarVim via NVIM_APPNAME=$APPNAME.
exec env NVIM_APPNAME="$APPNAME" "$NVIM_BIN" "\$@"
EOF
  fi
  chmod +x "$LAUNCHER"
